...
- **Container registry** - optional. The module manages a regional Artifact Registry by default (`create_artifact_registry = true`, see [`infra/variables.tf`](infra/variables.tf)). Set `create_artifact_registry = false` to reuse a registry managed by another stack; in that mode `artifact_registry_url` and the per-image repository outputs stay `null`, so point the Helm chart at the external registry via `gateway.image.repository`, `workers.common.image.repository`, and `config.image.repository`. The worker-sidecar uses the chart's `ghcr.io/superlinked/sie-server-sidecar` default.
